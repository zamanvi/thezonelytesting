...
<style>
    ...
    .glass { background:rgba(255,255,255,.12); backdrop-filter:blur(10px); border:1px solid rgba(255,255,255,.22); }
    ...
    .lift:hover { transform:translateY(-4px);box-shadow:0 20px 40px -10px rgba(0,0,0,.13); }
    .sh { position:relative;display:inline-block; }
    .sh::after { content:'';position:absolute;width:48px;height:3px;background:#2563eb;bottom:-8px;left:50%;transform:translateX(-50%);border-radius:9999px; }
    ...
    .booking-body { max-height:0;overflow:hidden;transition:max-height .45s ease-out,opacity .3s ease;opacity:0; }
...
@section('content')
<div class="mt-16 sm:mt-20 bg-white min-h-screen">

{{-- ── IN-PAGE NAV ──────────────────────────────────────────────── --}}
<div class="sticky top-16 sm:top-20 z-40 bg-white border-b border-slate-100 shadow-sm">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
        <nav class="hidden md:flex items-center gap-6 text-sm font-semibold text-slate-600">
            @if($activeServices->count())<a href="#services" class="hover:text-blue-600 transition">Services</a>@endif
            @if($activeServices->count())<a href="#pricing" class="hover:text-blue-600 transition">Pricing</a>@endif
            @if($user->educations->count() || $user->memberships->count())<a href="#background" class="hover:text-blue-600 transition">About</a>@endif
            <a href="#contact" class="hover:text-blue-600 transition">Contact</a>
        </nav>
        @if($callNumber)
        <a href="tel:{{ $callNumber }}"
           class="hidden md:flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2 rounded-full font-semibold text-sm transition">
            <i class="fas fa-phone text-xs"></i> Call Now
        </a>
        @endif
        <a href="{{ route('frontend.service.all') }}" class="md:hidden text-xs text-slate-500 hover:text-blue-600 flex items-center gap-1">
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 py-10 sm:py-14 md:py-20">
        <div class="flex flex-col md:flex-row items-center gap-10 md:gap-14">
                ...
                <div class="glass rounded-3xl p-6 text-center">
                    <div class="relative inline-block">
                        <img src="{{ $user->profile_photo }}" alt="{{ $user->name }}"
                             class="w-36 h-36 object-cover rounded-2xl border-4 border-white/30 shadow-xl mx-auto"
                             onerror="this.style.display='none';this.nextElementSibling.style.display='flex';">
                        <div style="display:none;" class="w-36 h-36 bg-blue-500/40 border-4 border-white/30 rounded-2xl mx-auto items-center justify-center text-white font-black text-4xl shadow-xl">
            ...
            <div class="flex-1 text-center md:text-left">
                ...
                <h2 class="text-yellow-300 text-xl sm:text-2xl font-bold mt-2">{{ $user->name }}</h2>
                ...
                @if($reviewCount)
                <a href="#reviews"
                   class="mt-4 inline-flex items-center gap-2 justify-center md:justify-start text-white/80 hover:text-white text-sm font-semibold transition group">
...
{{-- ── TRUST BAR ────────────────────────────────────────────────── --}}
@if($user->memberships->count() || $user->educations->count())
<div class="bg-slate-800 overflow-hidden py-3.5">
    <div class="marquee-track">
        @php $trustItems = collect(); @endphp
        @foreach($user->memberships as $m)
            @php $trustItems->push(['icon'=>'fas fa-certificate text-yellow-400','text'=> $m->name]); @endphp
        @endforeach
        @foreach($user->educations as $edu)
            @php $trustItems->push(['icon'=>'fas fa-graduation-cap text-blue-400','text'=> $edu->degree . ($edu->institution ? ' — '.$edu->institution : '')]); @endphp
        @endforeach
        @php if($trustItems->isEmpty()) $trustItems->push(['icon'=>'fas fa-shield-alt text-emerald-400','text'=>'Verified on Zonely']); @endphp
        @foreach([0,1] as $_)
        <div class="flex items-center gap-10 text-sm font-semibold text-white px-8">
...
<div class="max-w-6xl mx-auto px-4 sm:px-6 md:px-8 py-10 md:py-14 space-y-14">

    {{-- ── SERVICES ─────────────────────────────────────────────── --}}
    @if($activeServices->count() || count($tags))
    <section id="services">
        <div class="text-center mb-10">
            <h3 class="font-bold text-2xl sm:text-3xl sh">Professional Services</h3>
            <p class="text-slate-500 mt-7">Everything {{ $user->name }} offers</p>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 {{ $activeServices->count() >= 4 ? 'lg:grid-cols-4' : '' }} gap-4 md:gap-5">
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 lift text-center">
                ...
                @if($svc->description)
                <p class="text-xs text-slate-500 mt-2">{{ Str::limit($svc->description, 60) }}</p>
                @endif
                @if($svc->price)
                <p class="mt-3 text-blue-700 font-black text-lg">${{ $svc->price }}</p>
                @endif
            </div>
            @empty
    ...
    {{-- ── PRICING ──────────────────────────────────────────────── --}}
    @if($activeServices->where('price', '!=', null)->count())
    <section id="pricing">
        <div class="text-center mb-10">
            <h3 class="font-bold text-2xl sm:text-3xl sh">Transparent Pricing</h3>
            ...
            <div class="bg-white rounded-2xl overflow-hidden shadow-sm border {{ $loop->first ? 'border-2 border-blue-500' : 'border-slate-100' }}">
                ...
                <button onclick="toggleAccordion(this)" class="w-full flex items-center justify-between px-6 py-5 text-left hover:bg-slate-50 transition">
                    ...
                    <div class="text-right flex-shrink-0 ml-4">
                        @if($svc->price)
                        <div class="text-2xl font-black text-blue-700 leading-none">${{ $svc->price }}</div>
    ...
    <section id="reviews">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <div class="bg-gradient-to-br from-slate-50 to-white border border-slate-100 rounded-2xl p-6 lift">
    ...
    @if($user->educations->count() || $user->memberships->count() || $user->about || $user->bio)
    <section id="background">
        <div class="text-center mb-10">
            <h3 class="font-bold text-2xl sm:text-3xl sh">Professional Background</h3>
    ...
    <section id="location">
        <div class="text-center mb-10">
            <h3 class="font-bold text-2xl sm:text-3xl sh">Find Us</h3>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6 items-stretch">
            <div class="rounded-2xl overflow-hidden shadow-md" style="min-height:260px;">
...
{{-- ── FOOTER STRIP ─────────────────────────────────────────────── --}}
<div class="bg-slate-900 text-slate-400 py-6 text-center text-xs">
    <span>© {{ date('Y') }} Zonely · All Rights Reserved ·</span>
    <a href="{{ route('frontend.home') }}" class="hover:text-white transition ml-1">Find more professionals →</a>
</div>
...
<script>
    function toggleAccordion(btn) {
        ...
        document.querySelectorAll('.accordion-content.open').forEach(el => { if(el!==content) el.classList.remove('open'); });
