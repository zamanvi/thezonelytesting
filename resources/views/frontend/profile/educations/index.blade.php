...
<div class="pb-10 max-w-2xl mx-auto">
    ...
    <div class="mb-6 flex items-center justify-between gap-3">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl font-bold text-gray-900">Education & Certifications</h1>
                <p class="text-xs text-gray-500 mt-0.5">Shown in the Professional Background section</p>
            </div>
        </div>
        ...
        <div class="flex items-center justify-between gap-4">
            <div class="flex items-center gap-3 min-w-0">
                <div class="w-10 h-10 bg-teal-50 rounded-xl flex items-center justify-center shrink-0">
                    <i class="fa-solid fa-graduation-cap text-teal-600 text-sm"></i>
                </div>
                <div class="min-w-0">
                    <p class="font-bold text-slate-800 text-sm truncate">{{ $item->degree }}</p>
                    <p class="text-xs text-slate-400 mt-0.5">
                        {{ $item->institution ?? '' }}{{ ($item->institution && $item->passing_year) ? ' · ' : '' }}{{ $item->passing_year ?? '' }}
                    </p>
                </div>
            </div>
            <div class="flex items-center gap-2 shrink-0">
                ...
                <form action="{{ route('user.educations.destroy', $item->id) }}" method="POST"
                      onsubmit="return confirm('Delete: {{ addslashes($item->degree) }}?')">
                    @csrf @method('DELETE')
                    <button type="submit"
    ...
    <div class="bg-white rounded-2xl border-2 border-dashed border-slate-200 p-10 text-center">
        <div class="w-14 h-14 bg-teal-50 rounded-2xl flex items-center justify-center mx-auto mb-4">
            <i class="fa-solid fa-graduation-cap text-teal-400 text-2xl"></i>
        </div>
        <p class="font-bold text-slate-700 mb-1">No credentials yet</p>
        <p class="text-sm text-slate-400 mb-5">Degrees, certifications, and professional qualifications build trust.</p>
        <a href="{{ route('user.educations.create') }}"
           class="inline-flex items-center gap-2 px-5 py-2.5 bg-teal-700 hover:bg-teal-800 text-white font-bold rounded-2xl text-sm transition">
            <i class="fa-solid fa-plus text-xs"></i> Add First Credential
        </a>
    </div>
    @endforelse
