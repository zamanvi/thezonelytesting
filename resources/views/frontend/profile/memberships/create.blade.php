...
<div class="pb-10 max-w-2xl mx-auto">
    ...
    <form action="{{ route('user.memberships.store') }}" method="POST" class="space-y-4">
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
            ...
            <input type="text" name="name" value="{{ old('name') }}" required placeholder="e.g. American Institute of CPAs (AICPA)" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-teal-600 focus:ring-2 focus:ring-teal-50 transition">
        </div>
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
