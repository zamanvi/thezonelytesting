        ...
        <div class="col-lg-12">
            ...
            <div class="iq-card">
                <div class="iq-card-header d-flex justify-content-between">
                    <h4 class="card-title">Edit Policy for <strong>{{ $vehicle->name }}</strong></h4>
                ...
                <div class="iq-card-body">
                    <form action="{{ route('admin.policies.update', [$vehicle->id, $policy->id]) }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <select name="policy_type" id="policy_type" class="form-control" required>
                                ...
                                @foreach(['Liability', 'Comprehensive', 'Full Coverage'] as $type)
                                    <option value="{{ $type }}" 
                                        {{ old('policy_type', $policy->policy_type) == $type ? 'selected' : '' }}>
                                        {{ $type }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="status" id="status" class="form-control" required>
                                ...
                                @foreach(['Active', 'Expired', 'Cancelled'] as $status)
                                    <option value="{{ $status }}" 
                                        {{ old('status', $policy->status) == $status ? 'selected' : '' }}>
                                        {{ $status }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="notes" id="notes" class="form-control" rows="3">{{ old('notes', $policy->notes) }}</textarea>
                            @error('notes')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                        ...
                        <div class="mt-3">
                            <button type="submit" class="btn btn-primary">
                                <i class="ri-save-line"></i> Update Policy
                            </button>
